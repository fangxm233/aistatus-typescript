...
| `config.ts` | config parser | Load flat or mode-aware nested gateway configs and auto-discover env-based defaults |
| `server.ts` | HTTP runtime | Serve proxy traffic plus `/health`, `/status`, `/usage`, and `/mode` endpoints |
