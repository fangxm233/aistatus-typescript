...
| `server.ts` | HTTP runtime | Serve proxy traffic plus `/health`, `/status`, `/usage`, and `/mode` endpoints; enforces gateway auth, exposes `reloadConfig()` for in-place hot reload, and forwards persisted usage records to the uploader |
